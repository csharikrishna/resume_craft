...
export default function Builder() {
  const { resumeData, setResumeData, selectedTemplate, setSelectedTemplate, jobRole, setJobRole, saveResume, currentResumeId, isSaving, lastSaved } = useResume();
  ...
  useEffect(() => {
    if (!resumeData.personal?.name) return;

    const timer = setTimeout(async () => {
      try {
        setAutoSaveStatus('saving');
        await saveResume(resumeData.personal?.name ? `${resumeData.personal.name}'s Resume` : 'My Resume');
        setAutoSaveStatus('saved');
      } catch (err) {
        setAutoSaveStatus('error');
        setTimeout(() => setAutoSaveStatus('saved'), 5000);
      }
    }, 30000);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to process file. Ensure it is a valid PDF or DOCX.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleOptimize = async () => {
    ...
    if (!resumeData.personal?.name) return setError('Please add your name first');
    setAiLoading(true);
    setError('');
    try {
  ...
  const inputCls = "w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent transition-colors";
                  ...
                  <div>
                    ...
                    {activeSection === 'personal' && (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          {[['email', 'Email'], ['phone', 'Phone'], ['location', 'Location'], ['linkedin', 'LinkedIn URL'], ['portfolio', 'Portfolio/Website']].map(([f, l]) => (
                            <div key={f} className={f === 'email' ? 'col-span-2 md:col-span-1' : ''}>
                    ...
                    {activeSection === 'summary' && (
                      <div className="space-y-3">
                        ...
                        <p className="text-xs text-gray-500">Write a compelling 2-3 sentence summary (optional)</p>
                        <textarea
                          value={resumeData.summary || ''}
                          onChange={e => setResumeData({ ...resumeData, summary: e.target.value })}
                          placeholder="Skilled developer with 5+ years of experience building scalable web applications..."
                          rows={5}
                          className={inputCls + ' resize-none'}
                        />
                      </div>
                    ...
                    {activeSection === 'experience' && (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={addExperience} className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900">
                        ...
                        {(resumeData.experience || []).map((exp, idx) => (
                          <div key={idx} className="border border-gray-200 rounded-xl p-3 space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {[['title', 'Job Title *'], ['company', 'Company *'], ['location', 'Location'], ['start', 'Start Date *'], ['end', 'End Date']].map(([f, l]) => (
                                <div key={f}>
                                  <label className={labelCls}>{l}</label>
                                  <input
                                    value={exp[f] || ''}
                                    onChange={e => updateExperience(idx, f, e.target.value)}
                                    className={inputCls}
                                    placeholder={f === 'start' || f === 'end' ? 'Jan 2022' : l}
                                    type={f.includes('Date') ? 'month' : 'text'}
                                  />
                                </div>
                              ))}
                            ...
                            <div>
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <button onClick={() => addBullet(idx)} className="text-slate-700 text-xs hover:text-slate-900 font-medium">+ Add</button>
                              </div>
                              {(exp.bullets || []).length === 0 && (
                                <button onClick={() => addBullet(idx)} className="w-full text-xs text-gray-400 border border-dashed border-gray-200 rounded-lg py-2 hover:border-slate-300 hover:text-slate-500 transition-colors">
                                  Add bullet points describing your achievements
                                </button>
                              )}
                              {(exp.bullets || []).map((b, bi) => (
                                <div key={bi} className="flex gap-1 mb-2">
                                  <input value={b} onChange={e => updateBullet(idx, bi, e.target.value)} className={inputCls} placeholder="Achieved X by doing Y..." />
                        ...
                        {(resumeData.experience || []).length === 0 && (
                          <button onClick={addExperience} className="w-full border-2 border-dashed border-gray-200 rounded-xl p-6 text-gray-400 text-sm hover:border-slate-300 hover:text-slate-500 transition-colors font-medium">
                            + Add your first job
                          </button>
                        )}
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900"
                          >
                        ...
                        {(resumeData.education || []).map((edu, idx) => (
                          <div key={idx} className="border border-gray-200 rounded-xl p-3 space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {[['degree', 'Degree / Course *'], ['institution', 'College / University *'], ['year', 'Graduation Year'], ['gpa', 'GPA (optional)']].map(([f, l]) => (
                                <div key={f}>
                                  <label className={labelCls}>{l}</label>
                                  <input
                                    value={edu[f] || ''}
                                    onChange={e => { const ed = [...resumeData.education]; ed[idx] = { ...ed[idx], [f]: e.target.value }; setResumeData({ ...resumeData, education: ed }); }}
                                    className={inputCls}
                                    placeholder={f === 'year' ? '2023' : l}
                    ...
                    {activeSection === 'skills' && (
                      <div className="space-y-4">
                        ...
                        {[['technical', 'Technical Skills'], ['tools', 'Tools & Technologies'], ['soft', 'Soft Skills']].map(([f, l]) => (
                          <div key={f}>
                            <label className={labelCls}>{l} (comma separated)</label>
                            <input
                              ...
                              className={inputCls}
                              placeholder={f === 'technical' ? "Python, JavaScript, React" : f === 'tools' ? "Git, Docker, AWS" : "Communication, Leadership, Problem Solving"}
                            />
                            <p className="text-xs text-gray-400 mt-1">{(resumeData.skills?.[f] || []).length} skill{(resumeData.skills?.[f] || []).length !== 1 ? 's' : ''} added</p>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {activeSection === 'projects' && (
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => setResumeData({ ...resumeData, projects: [...(resumeData.projects || []), { name: '', description: '', link: '', tech: [] }] })} className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900">
                            <Plus className="h-3.5 w-3.5" /> Add
                          </button>
                        </div>
                        {(resumeData.projects || []).map((p, idx) => (
                          <div key={idx} className="border border-gray-200 rounded-xl p-3 space-y-2">
                            ...
                            {[['name', 'Project Name *'], ['description', 'Description *'], ['link', 'Live Link (optional)']].map(([f, l]) => (
                              <div key={f}><label className={labelCls}>{l}</label><input
                                value={p[f] || ''}
                                onChange={e => { const pr = [...resumeData.projects]; pr[idx] = { ...pr[idx], [f]: e.target.value }; setResumeData({ ...resumeData, projects: pr }); }}
                                className={inputCls}
                                placeholder={l}
                                type={f === 'link' ? 'url' : 'text'}
                              /></div>
                            ))}
                            <div><label className={labelCls}>Technologies (comma separated)</label><input
                              value={(p.tech || []).join(', ')}
                              onChange={e => { const pr = [...resumeData.projects]; pr[idx] = { ...pr[idx], tech: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }; setResumeData({ ...resumeData, projects: pr }); }}
                              className={inputCls}
                              placeholder="React, Node.js, MongoDB" /></div>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {activeSection === 'achievements' && (
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <h3 className="font-semibold text-gray-800 text-sm">Achievements & Awards</h3>
                          <button onClick={() => setResumeData({ ...resumeData, achievements: [...(resumeData.achievements || []), ''] })} className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900"><Plus className="h-3.5 w-3.5" /> Add</button>
                        </div>
                        <p className="text-xs text-gray-500">Highlight notable achievements, awards, or recognitions</p>
                        {(resumeData.achievements || []).map((a, idx) => (
                          <div key={idx} className="flex gap-2">
                            <input
                              value={a}
                              onChange={e => { const ac = [...resumeData.achievements]; ac[idx] = e.target.value; setResumeData({ ...resumeData, achievements: ac }); }}
                              className={inputCls}
                              placeholder="E.g., Employee of the Month, Published Research..." />
                    ...
                    {activeSection === 'certifications' && (
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => setResumeData({ ...resumeData, certifications: [...(resumeData.certifications || []), { name: '', issuer: '', year: '' }] })} className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900"><Plus className="h-3.5 w-3.5" /> Add</button>
                        </div>
                        {(resumeData.certifications || []).map((c, idx) => (
                          <div key={idx} className="border border-gray-200 rounded-xl p-3 space-y-2">
                            ...
                            {[['name', 'Certificate Name *'], ['issuer', 'Issuing Organization'], ['year', 'Year']].map(([f, l]) => (
                              <div key={f}><label className={labelCls}>{l}</label><input
                                value={c[f] || ''}
                                onChange={e => { const ce = [...resumeData.certifications]; ce[idx] = { ...ce[idx], [f]: e.target.value }; setResumeData({ ...resumeData, certifications: ce }); }}
                                className={inputCls}
                                placeholder={l}
                                type={f === 'year' ? 'number' : 'text'}
                              /></div>
                    ...
                    {activeSection === 'languages' && (
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            onClick={() => setResumeData({ ...resumeData, languages: [...(resumeData.languages || []), { language: '', proficiency: 'Intermediate' }] })}
                            className="flex items-center gap-1 text-slate-700 text-xs font-medium hover:text-slate-900">
                            <Plus className="h-3.5 w-3.5" /> Add
                          </button>
                        </div>
                        <p className="text-xs text-gray-500">List the languages you speak and your proficiency level</p>
                        {(resumeData.languages || []).map((lang, idx) => (
                          <div key={idx} className="flex gap-2">
                            <input
                              value={lang.language || ''}
                              onChange={e => { const l = [...resumeData.languages]; l[idx] = { ...l[idx], language: e.target.value }; setResumeData({ ...resumeData, languages: l }); }}
                              className={inputCls}
                              placeholder="E.g., English, Hindi, Spanish..." />
                            <select
                              value={lang.proficiency || 'Intermediate'}
                              onChange={e => { const l = [...resumeData.languages]; l[idx] = { ...l[idx], proficiency: e.target.value }; setResumeData({ ...resumeData, languages: l }); }}
                              className="text-sm border border-gray-200 rounded-lg px-2 focus:outline-none focus:ring-2 focus:ring-slate-500 flex-shrink-0">
                              <option>Basic</option>
                              <option>Intermediate</option>
                              <option>Fluent</option>
                              <option>Native</option>
                            </select>
                            <button onClick={() => setResumeData({ ...resumeData, languages: resumeData.languages.filter((_, i) => i !== idx) })} className="text-red-400 hover:text-red-600 flex-shrink-0"><X className="h-4 w-4" /></button>
                          </div>
                        ))}
                      </div>
                    )}
