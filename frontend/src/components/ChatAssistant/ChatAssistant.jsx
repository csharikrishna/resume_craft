  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 bg-slate-700 text-white rounded-full p-4 shadow-lg hover:bg-slate-800 transition-all hover:scale-105 z-40 flex items-center gap-2"
      >
        <MessageCircle className="h-5 w-5" />
        <span className="text-sm font-semibold hidden sm:block">AI Coach</span>
        {remaining !== null && remaining > 0 && (
          <span className="bg-white text-slate-700 text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
            {remaining}
          </span>
        )}
      </button>
            ...
            {!reply && !error && (
              <div className="space-y-3">
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                  <p className="text-xs text-amber-700 font-medium mb-1">💡 How the AI coach works</p>
                ...
                <div className="flex flex-wrap gap-2">
                  {QUICK.map(q => (
                    <button
                      ...
                      className="bg-gray-100 hover:bg-amber-50 hover:text-amber-700 hover:border-amber-200 border border-gray-200 text-gray-700 text-xs px-3 py-1.5 rounded-full transition-colors disabled:opacity-40"
                    >
                      {q}
                    </button>
                  ))}
            ...
            {reply && (
              <div className="space-y-3">
                ...
                <div className="flex justify-end">
                  <div className="bg-slate-700 text-white rounded-2xl rounded-tr-sm px-3 py-2 max-w-xs">
                    <p className="text-xs leading-relaxed">{reply.question}</p>
                  </div>
                </div>
                {/* AI answer */}
                <div className="flex gap-2">
                  <div className="bg-slate-700 rounded-full p-1.5 h-fit flex-shrink-0">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-700 font-medium disabled:opacity-40 disabled:cursor-not-allowed"
                >
            ...
            <div className="border-t border-gray-100 p-3">
              <div className="flex gap-2">
                <input
                  ...
                  disabled={loading || remaining === 0}
                  className="flex-1 text-xs border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent disabled:bg-gray-50 placeholder-gray-400"
                />
                <button
                  onClick={() => send()}
                  disabled={!message.trim() || loading || remaining === 0}
                  className="bg-slate-700 text-white rounded-xl p-2.5 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
